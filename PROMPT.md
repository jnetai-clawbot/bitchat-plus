...
this should not require location to be turnt on and should work with just bluetooth and operate the same

Current old BitChat (that uses location) source files can be found /home/jay/Documents/Scripts/Mobile-Apps/Bitchat/Source/bitchat-android

BitChat-Plus should project location /home/jay/Documents/Scripts/Mobile-Apps/Bitchat/Source/BitChat-Plus

remake this app (using github actions and workflows with a unique package name of BitChat-plus that dosnt require location to be on and does not use external services (where possable) it must still work as a privacy messenger app but without google location and any tracking or analitics or usage data being sent home or to 3rd party service or server withiut breaking the functionality or ability to work, it must use just blutooth alone

changhe all the green colours in the app (green theme) to a nice Blue colour, allow optional location that can be enabled manually if a user wants it on to find location of a user if both users have this option on then both maybe able to see there own and the user they are talking to location (aswell as a compas and distance so both may find each other), allow location to be turned off and on without causing any crash or error, if available simply show and if not then hide locator / compas on screen section.

if location is turned on then auto show user location (not own i mean the uyser that enabled location) add a Turn on location toggle button that is used to enable / disable location regardless if enabled on device, if enabled on app it should proompt to allow (permissions) for device location so that it may work.
