...
import com.bitchat.plus.onboarding.BluetoothCheckScreen
import com.bitchat.plus.onboarding.BluetoothStatus
import com.bitchat.plus.onboarding.BluetoothStatusManager
import com.bitchat.plus.onboarding.BatteryOptimizationManager
import com.bitchat.plus.onboarding.BatteryOptimizationPreferenceManager
import com.bitchat.plus.onboarding.BatteryOptimizationScreen
import com.bitchat.plus.onboarding.BatteryOptimizationStatus
import com.bitchat.plus.onboarding.BackgroundLocationPermissionScreen
import com.bitchat.plus.onboarding.InitializationErrorScreen
import com.bitchat.plus.onboarding.InitializingScreen
import com.bitchat.plus.onboarding.LocationCheckScreen
import com.bitchat.plus.onboarding.LocationStatus
import com.bitchat.plus.onboarding.LocationStatusManager
import com.bitchat.plus.onboarding.OnboardingCoordinator
import com.bitchat.plus.onboarding.OnboardingState
import com.bitchat.plus.onboarding.PermissionExplanationScreen
import com.bitchat.plus.onboarding.PermissionManager
import com.bitchat.plus.ui.ChatScreen
import com.bitchat.plus.ui.ChatViewModel
import com.bitchat.plus.ui.OrientationAwareActivity
import com.bitchat.plus.ui.theme.BitchatTheme
import com.bitchat.plus.wifiaware.WifiAwareController
import com.bitchat.plus.nostr.PoWPreferenceManager
...
class MainActivity : OrientationAwareActivity() {
    ...
    private val forceFinishReceiver = object : android.content.BroadcastReceiver() {
        override fun onReceive(context: android.content.Context, intent: android.content.Intent) {
            if (intent.action == com.bitchat.plus.util.AppConstants.UI.ACTION_FORCE_FINISH) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val filter = android.content.IntentFilter(com.bitchat.plus.util.AppConstants.UI.ACTION_FORCE_FINISH)
        if (android.os.Build.VERSION.SDK_INT >= 33) {
            registerReceiver(
                forceFinishReceiver,
                filter,
                com.bitchat.plus.util.AppConstants.UI.PERMISSION_FORCE_FINISH,
                null,
        ...
        com.bitchat.plus.service.AppShutdownCoordinator.cancelPendingShutdown()
        ...
        meshService = com.bitchat.plus.service.MeshServiceHolder.getOrCreate(applicationContext)
        unifiedMeshService = com.bitchat.plus.service.MeshServiceHolder.getUnifiedOrCreate(applicationContext)
        ...
        when (onboardingState) {
            ...
            OnboardingState.LOCATION_CHECK -> {
                LocationCheckScreen(
                    ...
                    onSkip = {
                        mainViewModel.skipLocationCheck()
    ...
    private fun checkBluetoothAndProceed() {
        ...
        val bleRequired = try { com.bitchat.plus.ui.debug.DebugPreferenceManager.getBleEnabled(true) } catch (_: Exception) { true }
        if (!bleRequired) {
        ...
        lifecycleScope.launch {
            ...
            if (permissionManager.isFirstTimeLaunch()) {
                ...
                if (permissionManager.needsBackgroundLocationPermission() &&
                    ...
                    !com.bitchat.plus.onboarding.BackgroundLocationPreferenceManager.isSkipped(this@MainActivity)
                ) {
                    mainViewModel.updateOnboardingState(OnboardingState.BACKGROUND_LOCATION_EXPLANATION)
                } else {
     ...
     * Location is OPTIONAL in BitChat Plus - skip if disabled.
     */
    private fun checkLocationAndProceed() {
        ...
        // Check if user has previously skipped location check
        if (mainViewModel.isLocationCheckSkipped.value) {
            checkBatteryOptimizationAndProceed()
            return
        }
        ...
        when (mainViewModel.locationStatus.value) {
            ...
            LocationStatus.DISABLED -> {
                // Show location screen but allow skip - location is optional
                mainViewModel.updateOnboardingState(OnboardingState.LOCATION_CHECK)
                mainViewModel.updateLocationLoading(false)
            }
            LocationStatus.NOT_AVAILABLE -> {
                // Location not available - skip and continue
                Log.w("MainActivity", "Location services not available, skipping")
                mainViewModel.skipLocationCheck()
                checkBatteryOptimizationAndProceed()
            }
        }
    ...
    private fun handleLocationEnabled() {
        ...
        com.bitchat.plus.wifiaware.WifiAwareController.startIfPossible()
        checkBatteryOptimizationAndProceed()
    }
    ...
    private fun handleLocationDisabled(message: String) {
        ...
        mainViewModel.updateLocationLoading(false)
        mainViewModel.updateLocationStatus(locationStatusManager.checkLocationStatus())
        // Location is optional, skip and continue
        mainViewModel.skipLocationCheck()
        checkBatteryOptimizationAndProceed()
    }
    ...
    private fun handleOnboardingComplete() {
        ...
        val bleRequired2 = try { com.bitchat.plus.ui.debug.DebugPreferenceManager.getBleEnabled(true) } catch (_: Exception) { true }
        when {
    ...
    private fun startMeshForegroundServiceBestEffort() {
        ...
        try {
            com.bitchat.plus.service.MeshForegroundService.start(applicationContext)
    ...
    private fun initializeApp() {
        lifecycleScope.launch {
            try {
                ...
                com.bitchat.plus.nostr.LocationNotesInitializer.initialize(this@MainActivity)
    ...
    private fun handleNotificationIntent(intent: Intent) {
        val shouldOpenPrivateChat = intent.getBooleanExtra(
            com.bitchat.plus.ui.NotificationManager.EXTRA_OPEN_PRIVATE_CHAT, 
            false
        )
        
        val shouldOpenGeohashChat = intent.getBooleanExtra(
            com.bitchat.plus.ui.NotificationManager.EXTRA_OPEN_GEOHASH_CHAT,
            false
        )
        
        when {
            shouldOpenPrivateChat -> {
                val peerID = intent.getStringExtra(com.bitchat.plus.ui.NotificationManager.EXTRA_PEER_ID)
                val senderNickname = intent.getStringExtra(com.bitchat.plus.ui.NotificationManager.EXTRA_SENDER_NICKNAME)
            ...
            shouldOpenGeohashChat -> {
                val geohash = intent.getStringExtra(com.bitchat.plus.ui.NotificationManager.EXTRA_GEOHASH)
                ...
                if (geohash != null) {
                    ...
                    val level = when (geohash.length) {
                        7 -> com.bitchat.plus.geohash.GeohashChannelLevel.BLOCK
                        6 -> com.bitchat.plus.geohash.GeohashChannelLevel.NEIGHBORHOOD
                        5 -> com.bitchat.plus.geohash.GeohashChannelLevel.CITY
                        4 -> com.bitchat.plus.geohash.GeohashChannelLevel.PROVINCE
                        2 -> com.bitchat.plus.geohash.GeohashChannelLevel.REGION
                        else -> com.bitchat.plus.geohash.GeohashChannelLevel.CITY // Default fallback
                    }
                    val geohashChannel = com.bitchat.plus.geohash.GeohashChannel(level, geohash)
                    val channelId = com.bitchat.plus.geohash.ChannelID.Location(geohashChannel)
