...
import com.bitchat.plus.ui.PrivateMessageOrigin
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.bitchat.plus.ui.DataManager,
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch {
            try {
                ...
                if (packet.type != com.bitchat.plus.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch
                ...
                com.bitchat.plus.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
        ...
        when (payload.type) {
            ...
            NoisePayloadType.FILE_TRANSFER -> {
                ...
                if (file != null) {
                    ...
                    val savedPath = com.bitchat.plus.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = BitchatMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
                    ...
                    if (!admitted) {
                        com.bitchat.plus.features.file.FileUtils.deleteStoredMediaPaths(
                            application,
                            listOf(savedPath)
                        )
                    }
