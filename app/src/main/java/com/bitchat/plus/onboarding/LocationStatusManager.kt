...
class LocationStatusManager(
    ...
    private fun setupLocationSettingsLauncher() {
        locationSettingsLauncher = activity.registerForActivityResult(
            ...
            if (isEnabled) {
                ...
                onLocationDisabled("Location services are optional for BitChat Plus. Bluetooth works without location.")
            }
        }
    }
    ...
    fun getStatusMessage(status: LocationStatus): String {
        return when (status) {
            ...
            LocationStatus.DISABLED -> "Location services are disabled. Location is optional for BitChat Plus."
