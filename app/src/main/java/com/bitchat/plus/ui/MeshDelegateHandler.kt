...
class MeshDelegateHandler(
    ...
    private suspend fun processPeerUpdate(mergedPeers: List<String>) {
        ...
        runCatching { com.bitchat.plus.services.MessageRouter.tryGetInstance()?.onPeersUpdated(mergedPeers) }

        // Clean up channel members who disconnected
        channelManager.cleanupDisconnectedMembers(mergedPeers, getMyPeerID())

        runCatching { com.bitchat.plus.services.AppStateStore.canonicalizePrivateChats() }
        ...
        state.getSelectedPrivateChatPeerValue()?.let { currentPeer ->
            ...
            if (canonical != currentPeer) {
                com.bitchat.plus.services.ConversationAliasResolver.unifyChatsIntoPeer(
                    state = state,
    ...
    private fun unifyChatsIntoPeer(targetPeerID: String, keysToMerge: List<String>) {
        com.bitchat.plus.services.ConversationAliasResolver.unifyChatsIntoPeer(state, targetPeerID, keysToMerge)
    }
        ...
        if (shouldSendReadReceipt) {
            ...
            try {
                val meshPeerID = ContactDirectory.resolve(senderConversationID).meshPeerID
                    ?: senderPeerID.takeIf {
                        com.bitchat.plus.services.ContactIdentityResolver.isMeshPeerId(it)
                    }
                if (meshPeerID != null &&
     ...
     */
    fun getPeerInfo(peerID: String): com.bitchat.plus.mesh.PeerInfo? {
