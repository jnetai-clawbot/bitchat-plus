...
class UnifiedMeshService(
    ...
    override fun sendFavoriteNotification(peerID: String, isFavorite: Boolean) {
        val myNpub = try {
            com.bitchat.plus.nostr.NostrIdentityBridge.getCurrentNostrIdentity(context)?.npub
        } catch (_: Exception) {
            null
        }
    ...
    private fun mergedPeerIDs(): List<String> {
        ...
        try { ids.addAll(com.bitchat.plus.services.AppStateStore.peers.value) } catch (_: Exception) { }
