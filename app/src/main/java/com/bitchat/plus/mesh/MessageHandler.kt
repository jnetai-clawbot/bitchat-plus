...
import com.bitchat.plus.favorites.FavoriteControlMessage
...
import com.bitchat.plus.sync.PacketIdUtil
...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
    ...
    suspend fun handleNoiseEncrypted(routed: RoutedPacket): Boolean {
        ...
        try {
            ...
            val noisePayload = com.bitchat.plus.model.NoisePayload.decode(decryptedData)
            ...
            when (noisePayload.type) {
                com.bitchat.plus.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                    // Decode TLV private message exactly like iOS
                    val privateMessage = com.bitchat.plus.model.PrivateMessagePacket.decode(noisePayload.data)
                ...
                com.bitchat.plus.model.NoisePayloadType.FILE_TRANSFER -> {
                    // Handle encrypted file transfer; generate unique message ID
                    val file = com.bitchat.plus.model.BitchatFilePacket.decode(noisePayload.data)
                    if (file != null) {
                        Log.d(TAG, "Encrypted file from $peerID: ${file.fileSize} bytes")
                        val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                        val savedPath = com.bitchat.plus.features.file.FileUtils.saveIncomingFile(appContext, file)
                        val message = BitchatMessage(
                            id = uniqueMsgId,
                            sender = delegate?.getPeerNickname(peerID) ?: "Unknown",
                            content = savedPath,
                            type = com.bitchat.plus.features.file.FileUtils.messageTypeForMime(file.mimeType),
                            timestamp = java.util.Date(packet.timestamp.toLong()),
                            isRelay = false,
                            isPrivate = true,
                            recipientNickname = delegate?.getMyNickname(),
                ...
                com.bitchat.plus.model.NoisePayloadType.PEER_STATE -> {
                ...
                com.bitchat.plus.model.NoisePayloadType.DELIVERED -> {
                ...
                com.bitchat.plus.model.NoisePayloadType.READ_RECEIPT -> {
                ...
                com.bitchat.plus.model.NoisePayloadType.VERIFY_CHALLENGE -> {
                    delegate?.onVerifyChallengeReceived(peerID, noisePayload.data, packet.timestamp.toLong())
                }
                com.bitchat.plus.model.NoisePayloadType.VERIFY_RESPONSE -> {
    ...
    private suspend fun sendDeliveryAck(messageID: String, senderPeerID: String) {
        try {
            ...
            val ackPayload = com.bitchat.plus.model.NoisePayload(
                type = com.bitchat.plus.model.NoisePayloadType.DELIVERED,
    ...
    suspend fun handleAnnounceWithResult(routed: RoutedPacket): AnnounceHandlingResult {
        ...
        try {
            val neighborsOrNull = com.bitchat.plus.services.meshgraph.GossipTLV.decodeNeighborsFromAnnouncementPayload(packet.payload)
        ...
        try {
            ...
            if (response != null) {
                ...
                val responsePacket = BitchatPacket(
                    ...
                    payload = response,
                    signature = null,
                    ttl = com.bitchat.plus.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS
                )
    ...
    private suspend fun handleBroadcastMessage(routed: RoutedPacket) {
        ...
        try {
            ...
            val isFileTransfer = com.bitchat.plus.protocol.MessageType.fromValue(packet.type) == com.bitchat.plus.protocol.MessageType.FILE_TRANSFER
            val file = com.bitchat.plus.model.BitchatFilePacket.decode(packet.payload)
            if (file != null) {
    ...
    private suspend fun handlePrivateMessage(packet: BitchatPacket, peerID: String) {
        try {
            val isFileTransfer = com.bitchat.plus.protocol.MessageType.fromValue(packet.type) ==
                com.bitchat.plus.protocol.MessageType.FILE_TRANSFER
    ...
    private fun handleFavoriteNotificationFromMesh(content: String, fromPeerID: String) {
        try {
            ...
            if (noiseKey != null) {
                com.bitchat.plus.favorites.FavoritesPersistenceService.shared.updatePeerFavoritedUs(noiseKey, control.isFavorite)
                if (control.npub != null) {
                    com.bitchat.plus.favorites.FavoritesPersistenceService.shared.updateNostrPublicKey(noiseKey, control.npub)
                    com.bitchat.plus.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(fromPeerID, control.npub)
                }

                val rel = com.bitchat.plus.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
                ...
                val sys = com.bitchat.plus.model.BitchatMessage(
                    sender = "system",
                    content = notice,
                ...
                try {
                    val conversationID = com.bitchat.plus.services.ContactDirectory
                        .canonicalConversationId(fromPeerID)
                    val sysPrivate = com.bitchat.plus.model.BitchatMessage(
                        sender = "system",
                        content = notice,
                        timestamp = java.util.Date(),
                        isRelay = false,
...
interface MessageHandlerDelegate {
    ...
    fun updatePeerInfoFromVerifiedAnnouncement(
        ...
        isVerified: Boolean,
        capabilities: com.bitchat.plus.model.PeerCapabilities? = null
    ): Boolean
